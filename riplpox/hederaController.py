...
class HederaController(object):
  ...
  def _get_flow_demand(self, dst_ip):
    ...
    #     if it matches, num_st_incoming_flows + 1
    #return 1/num_dst_incoming_flows
        ...
        #for each link in the path
          #if link.usage + flow.demand > link.capacity
              #flow fits = false
              #break
        #if flow fits
          # for each link, increase usage
          #assign i as the path self[flow_key]['assigned'] = i
          #return path
    #else just return based on ecmp 
